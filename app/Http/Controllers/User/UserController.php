...
class UserController extends Controller
{
    ...
    public function formRegister()
    {
        ...
        $dataRegion = Regency::all();
        $dataDistrict = District::all();
        $dataVillage = Village::all();
        // dd($dataRegion);
        // Arahkan ke file user/register.blade.php
        return view('user.register', compact('dataProvince'));
    }
    
    public function Getcountry(Request $request)
    {
        $dataRegion = Regency::where('province_id', $request->id_provinsi)->get();
        // $dataDistrict = District::where('regency_id', $request->id_regency)->get();
        // $dataVillage = Village::where('province_id', $request->id_provinsi)->first();
        if(!isset($dataRegion))
        {
            return array(
                'message' => "Gagal",
    ...
    public function Getkelurahan(Request $request)
    {
        // $dataRegion = Regency::where('province_id', $request->id_provinsi)->get();
        // $dataDistrict = District::where('regency_id', $request->id_regency)->get();
        $dataVillage = Village::where('district_id', $request->id_kecamatan)->get();
        // dd($dataDistrict);
        
        if(!isset($dataVillage))
        {
            return array(
                'message' => "Gagal",
                'succes' => false
